...
def task_2():
    ...
    query = EmployeeAttendance.select(
        fn.Min(EmployeeAttendance.e_time).alias('min_time'))
    ...
    query = EmployeeAttendance.select(EmployeeAttendance.employee_id).where(
        EmployeeAttendance.e_time == min_time[0]['min_time']).where(EmployeeAttendance.e_type == 1 and EmployeeAttendance.e_data == date.today()).limit(1)
    print_query(query)
